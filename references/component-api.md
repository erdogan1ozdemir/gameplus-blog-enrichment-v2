# Component API — `gameplus_blog_components.py` (v10.1)
...
- `verify_output(final, blog_type, n_games, expect_faq)` + `print_report` — zorunlu çıktı kontrolü (tek H1, meta yok, TLDR 3-6, info-card, **Editör Notu + Hatırlatma zorunlu**, madde listesi uyarısı, embed 16:9, oyun sayısı, PlayStation uyarısı).
...
## Bileşenler
- `render_tldr(items)` — "Hızlı Özet": #161616 kart + dönen sarı glow + doküman ikonu; maddeler sarı `•` + gri 16/24. 3-6 madde.
- `render_info_card([(label, value), ...])` — stat karoselleri: #0D0D0D kart; **değer üstte** New Science 24px SARI, etiket altta gri; **içerik ortalı**. Değer KISA/sayısal ve yazıdan gerçek bilgi (kural 15).
...
- `render_game_cell(name, meta=None, href=None)` — tablo Oyun hücresi: isim (+link, alt çizgisiz) + altında "Stüdyo · Yıl" 12px gri. Stüdyo/yıl KAYNAKLI olmalı (Steam API / resmi sayfa); bilinmiyorsa meta=None.
- `render_cta_paketler(h, d)` / `render_cta_oyunlar(h, d)` — glow'lu #161616 CTA kartı, ★ eyebrow, New Science başlık; buton id: **packages-button / games-button**.
